...
namespace MicrosoftEduGraphSamples.workflows
{
    internal class Submission
    {
        ...
        public void ReassignWorkflow()
        {
            ...
            assig = microsoft_graph_sdk.Assignment.Publish(graphClient, _config["classId"], assignmentId);
            ...
            {
                assig = microsoft_graph_sdk.Assignment.GetAssignment(graphClient, _config["classId"], assignmentId);
            ...
            var submissions = microsoft_graph_sdk.Submission.GetSubmissions(graphClient, _config["classId"], assignmentId);
            ...
            var submission = microsoft_graph_sdk.Submission.Submit(graphClient, _config["classId"], assignmentId, submissionId);
            ...
            while (submission.Result.Status.ToString() != "Submitted" && retries <= MAX_RETRIES) {
                submission = microsoft_graph_sdk.Submission.GetSubmission(graphClient, _config["classId"], assignmentId, submissionId);
            ...
            submission = microsoft_graph_sdk.Submission.Reassign(graphClient, _config["classId"], assignmentId, submissionId);
            Console.WriteLine($"{submission.Result.Id} - {submission.Result.Status}");
